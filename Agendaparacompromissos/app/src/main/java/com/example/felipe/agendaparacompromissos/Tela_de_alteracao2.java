...
public class Tela_de_alteracao2 extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Cursor cursor = banco_de_dados.carregaDadoById2(Integer.parseInt(codigo));
        ...
        String radio;
        radio = cursor.getString(cursor.getColumnIndexOrThrow(Manter_bd.temp));//paga valor cadastrado dos radiobutton selecionados  na tela de cadastrado

        if (radio.equals("1")) {// com base no valor cadastrado no bd os radiobutton sao marcados
            radiogroup.check(R.id.radioButton1);
            editText.setText(cursor.getString(cursor.getColumnIndexOrThrow(Manter_bd.temp2)));
        } else {
            if (radio.equals("2")) {
                ...
                if (radio.equals("3")) {
                    ...
                    editText2.setText(cursor.getString(cursor.getColumnIndexOrThrow(Manter_bd.temp2)));
                } else {
                    ...
                    if (qnt_participantes.trim().isEmpty()) {
                        ...
                        if (radioButton1.isChecked()) {
                            if (num_ocorrencias.trim().isEmpty()) {
                                ...
                                String radio = "1";
                                banco_de_dados.alteraRegistro(Integer.parseInt(codigo), data_inicio, hora_ini, hora_fim, local, descricao, tipo_de_evento, qnt_participantes, ocorrencia_selecionada, repeticoes_selecionada, radio,num_ocorrencias);
                            ...
                            banco_de_dados.alteraRegistro(Integer.parseInt(codigo), data_inicio, hora_ini, hora_fim, local, descricao, tipo_de_evento, qnt_participantes, ocorrencia_selecionada, repeticoes_selecionada, radio,sempre_repetir);
                            Toast.makeText(getApplicationContext(), "Alterado com sucesso", Toast.LENGTH_SHORT).show();
                            Intent intent = new Intent(Tela_de_alteracao2.this, Calendario.class);
                            startActivity(intent);
                            onPause();
                            ...
                            if (data_final.trim().isEmpty()) {
                                ...
                                String radio = "3";
                                banco_de_dados.alteraRegistro(Integer.parseInt(codigo), data_inicio, hora_ini, hora_fim, local, descricao, tipo_de_evento, qnt_participantes, ocorrencia_selecionada, repeticoes_selecionada, radio, data_final);
                                Toast.makeText(getApplicationContext(), "Alterado com sucesso", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(Tela_de_alteracao2.this, Calendario.class);
                                startActivity(intent);
                                onPause();
